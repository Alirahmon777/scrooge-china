...
const PrivacyPage = () => {
  return (
    <section className='flex-grow'>
      <h2 className='text-6xl font-bold text-white mb-[40px]'>Конфиденциальность</h2>
      <div className='flex flex-col gap-[60px] items-start'>
        {privacy_data.map(({ title, children, content }) => (
          <div className='text-2xl text-gray'>
            ...
            {content && <p dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(content, {ALLOWED_ATTR: ['target', 'class', 'href']}) }}></p>}
            <div className='flex flex-col gap-[60px] items-start'>
              {children &&
